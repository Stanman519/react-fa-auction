...
import {
  UPDATE_CONFIDENCE,
  defaultConfState,
} from "../reducers/ConfidenceReducer";
import {
...
export const getMatchups =
  (user: User, year: number = new Date().getFullYear()) =>
  async (dispatch: Function, getState: () => RootState): Promise<any> => {
    ...
    dispatch(
      updateUI({
        multiLoader: [...(getState().ui.multiLoader ?? []), "con-matchups"],
      }),
    );
    ...
    if (response.matchups.every((m) => m.pick)) {
      ...
      response.matchups.forEach((m) => {
        if (m.pick?.choice === m.left.id) m.chosenTeamLocal = m.left;
        else if (m.pick?.choice === m.right.id) m.chosenTeamLocal = m.right;
      });
    }
    ...
    dispatch(
      updateUI({
        multiLoader: [
          ...(getState().ui.multiLoader?.filter((l) => l !== "con-matchups") ??
            []),
        ],
      }),
    );
...
export const getError =
  ...
  async (dispatch: Function, getState: () => RootState): Promise<any> => {
    ...
    if (!x.success && typeof x.data === "string")
      dispatch(updateUI({ modal: "error", errorText: x.data }));
  };

export const getConfidenceResults =
  (year?: number) =>
  async (dispatch: Function, getState: () => RootState): Promise<any> => {
    dispatch(
      updateUI({
        multiLoader: [...(getState().ui.multiLoader ?? []), "con-results"],
      }),
    );

    let response = await GeneralApiSvc.getConfidenceResults(
      year ?? lastYear + 1,
    );
    const state = getState().confidence;
    ...
    if (year === -1) {
      ...
      for (let i = 0; i < response.length; i++) {
        ...
        let usersWithRank = response.filter(
          (user) => user.totalPoints === totalPoints,
        );
    ...
    dispatch(
      updateUI({
        multiLoader: [
          ...(getState().ui.multiLoader?.filter((l) => l !== "con-results") ??
            []),
        ],
      }),
    );
...
export const pickTeamInMatchup =
  (matchup: NflMatchup, pick?: NflTeam) =>
  async (dispatch: Function, getState: () => RootState): Promise<any> => {
...
export const setViewModeForPicks =
  (mode: MyPickViewMode) =>
  async (dispatch: Function, getState: () => RootState): Promise<any> => {
    const { confidence } = getState();
...
export const reorderConfidenceMatchups =
...
export const makePropChoice =
  (propId: number, choice: "A" | "B") =>
  async (dispatch: Function, getState: () => RootState): Promise<any> => {
    const { confidence } = getState();
  ...
  async (): Promise<any> => {
    ...
    let postBody: NflMatchup[] = matchups.map((m) => {
      return {
        ...
        chosenTeamLocal: {},
        week,
        left: teams.find((t) => t.id === m.left),
        right: teams.find((t) => t.id === m.right),
        pickable: true,
      } as NflMatchup;
    });
...
export const submitMyPicks =
  ...
  async (dispatch: Function, getState: () => RootState): Promise<any> => {
    ...
    if (response.success) {
      dispatch(
        updateUI({ button: undefined, modal: "confidence-submit-success" }),
      );
      const { props, matchups, picks } = confidence;
      let newMatchups = [...matchups];
      let newProps = [...props];
      newMatchups.forEach((m) => {
        m.choice = m.chosenTeamLocal?.id;
      });
      newProps.forEach((p) => {
...
export const getCommunityStats =
  ...
  async (dispatch: Function, getState: () => RootState): Promise<any> => {
    ...
    await dispatch(
      updateCofidence({ ...confidence, communityStats: response }),
    );
    dispatch(setViewModeForPicks("community-picks"));
  };

export const submitDemoPicks =
  (thisWeekPoints: ConfidenceWeekPointsMap) =>
  async (dispatch: Function, getState: () => RootState): Promise<any> => {
    const { confidence } = getState();
    const { year, week } = getState().confidence.matchups[0];
    const response = await GeneralApiSvc.getCommunityStats(year, week);
...
export const clearConfidenceStateBeforeNav =
  () =>
  async (dispatch: Function, getState: () => RootState): Promise<any> => {

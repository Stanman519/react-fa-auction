...
export const TeamCapDetails = ({ height }: { height: number }) => {
  const { selectedTeam, deadCap } = useSelector(
    (state: RootState) => state.deadCap,
  );
  ...
  const filterPlayersForYear = (year: number) => {
    ...
    let thisTeamThisYear = transactions.filter(
      (t) =>
        t.franchiseId === selectedTeam &&
        t.yearOfTransaction <= year &&
        t.yearOfTransaction + t.years > year,
    );
    let relevant = thisTeamThisYear.filter(
      (t) =>
        !thisTeamThisYear.some(
          (e) => e.amount == -t.amount && e.playerName == t.playerName,
        ),
    );
    return relevant;
  };
        ...
        {selectedTeam ? (
          <div>
            <Typography className="text-center" variant={"h6"}>
              {" "}
              {deadCap.find((t) => t.franchiseId === selectedTeam)?.team}'s Cap
              Adjustments{" "}
            </Typography>
                        ...
                        <List>
                          {players.map((t) => (
                            <div
                              ...
                              key={`${t.transactionId}-${y}`}
                            >
                              <ListItemAvatar>
                                <Avatar>{t.position}</Avatar>
                              </ListItemAvatar>
                              <ListItemText
                                primary={t.playerName}
                                secondary={`$${t.amount}`}
          ...
          <Typography variant={"h6"}>
            {" "}
            Select a team to view player details.{" "}
          </Typography>
        )}
      </CardContent>
    </Card>
